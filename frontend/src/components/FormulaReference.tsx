    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-3xl max-h-[80vh] overflow-hidden flex flex-col">
        <div className="p-4 border-b border-gray-200 flex justify-between items-center">
          <h2 className="text-lg font-bold text-gray-800">Formula Reference Library</h2>
          <button 
            onClick={onClose}
        ...
        <div className="p-4 border-b border-gray-200">
          <input
            ...
            className="w-full p-2 bg-gray-50 border border-gray-300 rounded text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filteredFormulas.map((formula, index) => (
                <div key={index} className="bg-gray-50 border border-gray-200 rounded-lg shadow-sm p-4">
                  <h3 className="text-blue-600 font-bold">{formula.name}</h3>
                  <div className="mt-2 p-2 bg-white border border-gray-200 rounded">
                    <div className="latex-content" dangerouslySetInnerHTML={{ __html: `\\(${formula.formula}\\)` }}></div>
                  </div>
                  <p className="mt-2 text-gray-600 text-sm">{formula.explanation}</p>
                </div>
              ))}
            </div>
          )}
