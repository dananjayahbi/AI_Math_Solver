      ...
      <div className="flex flex-col space-y-2 mb-4">
        <div className="flex items-center">
          <button
            ...
            className="flex items-center justify-between bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-md w-full"
          >
        ...
        <div className="flex items-center space-x-2">
          <label className="inline-flex items-center cursor-pointer">
            ...
            <span className="ms-3 text-sm font-medium text-white">Step-by-step solution</span>
          </label>
        </div>
      </div>

      {isOpen && (
        <div className="absolute mt-1 w-full bg-gray-800 rounded-md shadow-lg z-50">
          <ul className="py-1">
            {MATH_MODES.map((mode) => (
              <li
                key={mode.id}
                className={`px-4 py-2 hover:bg-slate-700 cursor-pointer ${
                  selectedMode === mode.id ? 'bg-slate-700' : ''
                }`}
